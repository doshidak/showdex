...
export const formatMatchupNhko = (
  ...
  try {
    ...
    if (typeof resultKoChance.chance === 'number' && resultKoChance.chance !== 1) {
      ...
      const decimalPlaces = chancePercentage.toFixed(1).endsWith('.0') ? 2 : 1;
      ...
      */
      ...
      // update (2024/07/19): should've just copied their KOChance formula lol v-v
      output.unshift(`${clamp(1, Math.round(resultKoChance.chance * 1000), 999) / 10}%`);
    }
